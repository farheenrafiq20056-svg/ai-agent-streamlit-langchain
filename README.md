...
- 🔎 **Web Search** — looks up current information (via the `ddgs` package, no API key needed)
...
## How it works
- `create_agent` (LangChain's current agent API) builds an agent that can call the 3 tools above.
- Behind the scenes it runs a loop: the LLM decides whether to call a tool, the tool result is fed back in, and this repeats until it has a final answer.
